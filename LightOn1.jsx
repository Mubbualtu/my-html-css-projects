import './LightOn1.css'

export default function LightOn1() {
  return (
    <div className="light-on-1">
      <img className="menu-slider" src="assets/vectors/MenuSlider13_x2.svg" />
      <div className="status-bar-iphone">
        <div className="time">
          <span className="time-1">
          9:41
          </span>
        </div>
        <div className="levels">
          <img className="cellular-connection" src="assets/vectors/CellularConnection2_x2.svg" />
          <img className="wifi" src="assets/vectors/Wifi14_x2.svg" />
          <img className="battery" src="assets/vectors/Battery13_x2.svg" />
        </div>
      </div>
      <div className="container">
        <div className="living-room-light-is-off">
        Living room light is off
        </div>
        <div className="toggle">
          <div className="knob">
          </div>
        </div>
      </div>
      <div className="fan-value">
        <div className="container-1">
          <div className="active-track">
            <div className="track-dot-1">
              <div className="dot">
              </div>
            </div>
            <div className="track-dot-2">
              <div className="dot-1">
              </div>
            </div>
            <div className="track-dot-3">
              <div className="dot-2">
              </div>
            </div>
            <div className="track-dot-4">
              <div className="dot-3">
              </div>
            </div>
            <div className="track-dot-5">
              <div className="dot-4">
              </div>
            </div>
          </div>
          <div className="handle">
            <div className="state-layer">
            </div>
          </div>
          <div className="inactive-track">
            <div className="track-dot-7">
              <div className="dot-5">
              </div>
            </div>
            <div className="track-dot-8">
              <div className="dot-6">
              </div>
            </div>
            <div className="track-dot-9">
              <div className="dot-7">
              </div>
            </div>
            <div className="track-dot-10">
              <div className="dot-8">
              </div>
            </div>
            <div className="track-dot-11">
              <div className="dot-9">
              </div>
            </div>
          </div>
        </div>
        <div className="value-indicator">
          <span className="label-text-1">
          50
          </span>
        </div>
      </div>
      <img className="group-1" src="assets/vectors/Group11_x2.svg" />
      <div className="go-back">
        <div className="container-2">
          <span className="label-text">
          Go back
          </span>
        </div>
      </div>
    </div>
  )
}